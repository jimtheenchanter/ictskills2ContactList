describe("Main View ", () => {
    ...
    describe("Filtering", () => {
        it("filteres the contacts by name", () => {
            ...
            //To iterate over the collection use the 'each()' command - cy.get(".card").each().
            cy.get(".card").each($el => {

                //Cypress uses the JQuery library for many of its underlying DOM querying.
                // For a consistent programming model use cy.wrap() to convert JQuery result
                // objects to Cypress objects.
                cy.wrap($el)
                    .find(".card-title")
                    .contains("la");
            });
    ...
    describe("Navigate to Contact page", () => {
        //it.only("goes to the correct view", () => { ::will runonly this test
